...
# 读取第一天的传感器分配数据
sensor_num_first_day_data = pd.read_csv("data/传感器布设优化/Four_day/sensor_first_day_" + str(N_k) + ".csv")
    ...
    if int(sensor_num) == 0:
        ...
        for w in choose_result_list:
            if len(vehicle_route_data[w]) == 0:
                ...
                vehicle_list.append(v)
                v.traj[day] = vehicle_route_data[w]
            no += 1

RESET_DAYS = [11, 18, 25]     # 需要重置的日期
# 全局变量：上一次重置的日期（初始设为 3，保证 4-9 全部累计）
last_reset_day = 3

# 用于存储“最近一次重置后”的轨迹索引
trajectories_after_reset = []
...
for day in range(4, 31):
    ...
    if day in RESET_DAYS:
        last_reset_day = day
        trajectories_after_reset.clear()  # 清空历史

    # 3. 把“今天新增的轨迹”追加到“重置后列表”
    for vehicle in vehicle_list:
        # 仅取 day 当天轨迹
        route_list = vehicle.traj.get(day, [])
        if route_list:
            trajectories_after_reset.append(route_list)

    # 4. 仅对“重置后轨迹”做 0/1 覆盖统计
    current_road_coverage_dic = defaultdict(int)
    covered = set()
    for route_list in trajectories_after_reset:
...
# 存储车辆轨迹
save_trajectories(vehicle_list, "data/传感器布设优化/车辆路线每周/" + str(N_k) + "_vehicle_trajectories.json")