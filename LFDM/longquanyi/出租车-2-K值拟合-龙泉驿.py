...
def fold_day_to_week(day_str: str, base_str: str = "2024-03-01") -> str | None:
    """
    把任意日期折叠到 base_str 起始的7天（base + delta%7），用于只保留 3.1-3.7。
    例：base=2024-03-01
      2024-03-08, 2024-03-15 -> 2024-03-01
      2024-03-09, 2024-03-16 -> 2024-03-02
    返回 YYYY-MM-DD
    """
    if day_str is None:
        return None
    ...
    # 1) 优先从字符串中提取 YYYY-MM-DD（最稳，兼容带时间的情况）
    m = _DATE_RE.search(s)
    if m:
        d = datetime.strptime(m.group(1), "%Y-%m-%d").date()
    else:
        # 2) 兜底解析（兼容 '2024/03/08'、'2024-03-08 12:00:00' 等）
    ...
    delta = (d - base).days
    folded = base + timedelta(days=(delta % 7))
    return folded.strftime("%Y-%m-%d")


def build_daily_car2edges_from_jsonl(jsonl_path: str, base_str: str = "2024-03-01", debug: bool = False):
    """
    读取 final_all_cabs.jsonl（每行一个 cab 的输出），返回：
      day2car2edges[folded_date][cab_id] = set(edge_id)

    JSONL 每行格式假设为：
      {"cab_id": "...", "trips":[{"date":"YYYY-MM-DD", "edge_seq":[...]} , ...]}

    关键改动：
      - date 会被折叠到 base_str 起始的7天（只保留 3.1-3.7）
    """
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for ln, line in enumerate(f, 1):
            ...
            for t in trips:
                raw_day = t.get("date", None)
                if not raw_day:
                    continue

                raw_days.add(str(raw_day))
                day = fold_day_to_week(raw_day, base_str=base_str)   # ✅ 这里折叠日期
                if not day:
                    continue
                folded_days.add(day)
    ...
    if debug:
        print("[DEBUG] raw unique days (sample):", sorted(list(raw_days))[:30])
        print("[DEBUG] folded days:", sorted(list(folded_days)))
