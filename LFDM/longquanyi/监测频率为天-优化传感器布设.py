...
import pickle
from subfunction_optimization import sensor_dis_model_one_day
import numpy as np
...
for day in dispatch_days:
    ...
    [sensor_num_first_day_dic, sensor_dispatch_dic] = sensor_dis_model_one_day(day, N_k, station_id_list, sensor_num_end_day_dic, edge_list, edge_length_dic, start_vehicle_num_every_day_dic, k_value_dic)
    ...
    for i in station_id_list:
        expected = round(sensor_num_first_day_dic[i])
...
# 存储车辆轨迹
save_trajectories(vehicle_list, "data/传感器布设优化/车辆路线每天/" + str(N_k) + "_vehicle_trajectories.json")