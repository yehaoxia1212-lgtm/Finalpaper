...
# 0) 配置：改这里
# =========================
EDGE_GEOJSON = r"D:\Finalpaper\LFDM\san\roadmap_taxi_san\edge.geojson"
ADJ_CSV      = r"D:\Finalpaper\LFDM\san\roadmap_taxi_san\adj_matrix.csv"
TAXI_CSV     = r"D:\Finalpaper\LFDM\san\data_taxi\data_taxi_san.csv"
...
# 输出目录
OUT_DIR = r"D:\Finalpaper\LFDM\san\data_taxi\final_clean_trips"
...
def main():
    ...
    cab_ids = sorted(pd.to_numeric(df[CAB_COL], errors="coerce").dropna().astype(int).unique().tolist())
    if ONLY_CAB is not None:
        cab_ids = [int(ONLY_CAB)] if int(ONLY_CAB) in cab_ids else []
